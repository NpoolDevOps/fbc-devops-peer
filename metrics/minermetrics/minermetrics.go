...
func NewMinerMetrics(cfg MinerMetricsConfig, paths []string) *MinerMetrics {
	mm := &MinerMetrics{
		...
		SectorTaskElapsed: prometheus.NewDesc(
			...
			[]string{"tasktype", "worker", "user"}, nil,
		),
		...
		SectorTaskProgress: prometheus.NewDesc(
			...
			[]string{"tasktype", "worker", "sector", "done", "user"}, nil,
		),
...
func (m *MinerMetrics) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(m.ForkBlocks, prometheus.CounterValue, float64(forkBlocks), username)
	ch <- prometheus.MustNewConstMetric(m.PastBlocks, prometheus.CounterValue, float64(pastBlocks), username)
	ch <- prometheus.MustNewConstMetric(m.FailedBlocks, prometheus.CounterValue, float64(failedBlocks), username)
	ch <- prometheus.MustNewConstMetric(m.BlockTookAvgMs, prometheus.CounterValue, float64(avgMs), username)
	ch <- prometheus.MustNewConstMetric(m.BlockTookMaxMs, prometheus.CounterValue, float64(maxMs), username)
	ch <- prometheus.MustNewConstMetric(m.BlockTookMinMs, prometheus.CounterValue, float64(minMs), username)
	ch <- prometheus.MustNewConstMetric(m.Blocks, prometheus.CounterValue, float64(len(tooks)), username)
	ch <- prometheus.MustNewConstMetric(m.MinerIsMaster, prometheus.CounterValue, minerIsMaster, username)
			...
			for _, task := range workerTasks {
				if task.Done {
					...
					ch <- prometheus.MustNewConstMetric(m.SectorTaskProgress, prometheus.CounterValue,
						float64(task.Elapsed), taskType, worker, task.Sector, "1", username)
				} else {
					concurrent += 1
					totalConcurrent += 1
					...
					ch <- prometheus.MustNewConstMetric(m.SectorTaskProgress, prometheus.CounterValue,
						float64(task.Elapsed), taskType, worker, task.Sector, "0", username)
				}
			}
			ch <- prometheus.MustNewConstMetric(m.SectorTaskElapsed, prometheus.CounterValue, float64(elapsed), taskType, worker, username)
			ch <- prometheus.MustNewConstMetric(m.SectorTaskDuration, prometheus.CounterValue, float64(duration), taskType, worker, username)
			ch <- prometheus.MustNewConstMetric(m.SectorTaskConcurrent, prometheus.CounterValue, float64(concurrent), taskType, worker, username)
			ch <- prometheus.MustNewConstMetric(m.SectorTaskDones, prometheus.CounterValue, float64(dones), taskType, worker, username)
		}
		ch <- prometheus.MustNewConstMetric(m.MinerSectorTaskConcurrent, prometheus.CounterValue, float64(totalConcurrent), taskType, username)
		ch <- prometheus.MustNewConstMetric(m.MinerSectorTaskDones, prometheus.CounterValue, float64(totalDones), taskType, username)
	}
	...
	for taskType, typedJobs := range jobs.Jobs {
		...
		for worker, job := range typedJobs {
			ch <- prometheus.MustNewConstMetric(m.SectorTaskWaitingElapsed, prometheus.CounterValue, float64(job.MaxWaiting), taskType, worker, username)
			ch <- prometheus.MustNewConstMetric(m.SectorTaskRunningElapsed, prometheus.CounterValue, float64(job.MaxRunning), taskType, worker, username)
			ch <- prometheus.MustNewConstMetric(m.SectorTaskRunning, prometheus.CounterValue, float64(job.Running), taskType, worker, username)
			ch <- prometheus.MustNewConstMetric(m.SectorTaskWaiting, prometheus.CounterValue, float64(job.Assigned), taskType, worker, username)
			totalRunning += job.Running
			totalWaiting += job.Assigned
		}
		ch <- prometheus.MustNewConstMetric(m.MinerSectorTaskRunning, prometheus.CounterValue, float64(totalRunning), taskType, username)
		ch <- prometheus.MustNewConstMetric(m.MinerSectorTaskWaiting, prometheus.CounterValue, float64(totalWaiting), taskType, username)
	}

	m.mutex.Lock()
	info := m.minerInfo
	m.mutex.Unlock()
	ch <- prometheus.MustNewConstMetric(m.Power, prometheus.CounterValue, float64(info.Power), username)
	ch <- prometheus.MustNewConstMetric(m.RawPower, prometheus.CounterValue, float64(info.Raw), username)
	ch <- prometheus.MustNewConstMetric(m.CommittedPower, prometheus.CounterValue, float64(info.Committed), username)
	ch <- prometheus.MustNewConstMetric(m.ProvingPower, prometheus.CounterValue, float64(info.Proving), username)
	ch <- prometheus.MustNewConstMetric(m.FaultyPower, prometheus.CounterValue, float64(info.Faulty), username)
	ch <- prometheus.MustNewConstMetric(m.MinerBalance, prometheus.CounterValue, float64(info.MinerBalance), username)
	ch <- prometheus.MustNewConstMetric(m.PrecommitDeposit, prometheus.CounterValue, float64(info.PrecommitDeposit), username)
	ch <- prometheus.MustNewConstMetric(m.InitialPledge, prometheus.CounterValue, float64(info.InitialPledge), username)
	ch <- prometheus.MustNewConstMetric(m.Vesting, prometheus.CounterValue, float64(info.Vesting), username)
	ch <- prometheus.MustNewConstMetric(m.Available, prometheus.CounterValue, float64(info.Available), username)
	ch <- prometheus.MustNewConstMetric(m.WorkerBalance, prometheus.CounterValue, float64(info.WorkerBalance), username)
	ch <- prometheus.MustNewConstMetric(m.ControlBalance, prometheus.CounterValue, float64(info.ControlBalance), username)
	for state, count := range m.sectorStat {
		ch <- prometheus.MustNewConstMetric(m.MinerTaskState, prometheus.CounterValue, float64(count), state, username)
	}
	ch <- prometheus.MustNewConstMetric(m.MinerSectorSizeGib, prometheus.CounterValue, float64(info.SectorSize), username)

	basefee, _ := lotusapi.ChainBaseFee(m.fullnodeHost)
	ch <- prometheus.MustNewConstMetric(m.MinerBaseFee, prometheus.CounterValue, basefee, username)
	...
	gpus := 0
	ch <- prometheus.MustNewConstMetric(m.MinerWorkers, prometheus.CounterValue, float64(len(workerInfos.Infos)), username)
	for worker, info := range workerInfos.Infos {
		ch <- prometheus.MustNewConstMetric(m.MinerWorkerGPUs, prometheus.CounterValue, float64(info.GPUs), worker, username)
		ch <- prometheus.MustNewConstMetric(m.MinerWorkerMaintaining, prometheus.CounterValue, float64(info.Maintaining), worker, username)
		ch <- prometheus.MustNewConstMetric(m.MinerWorkerRejectTask, prometheus.CounterValue, float64(info.RejectTask), worker, username)
		gpus += info.GPUs
	}
	ch <- prometheus.MustNewConstMetric(m.MinerGPUs, prometheus.CounterValue, float64(gpus), username)

	checkSectors := m.ml.GetCheckSectors()
	for deadline, sectors := range checkSectors {
		ch <- prometheus.MustNewConstMetric(m.MinerCheckSectorsGood, prometheus.CounterValue, float64(sectors.Good), fmt.Sprintf("%v", deadline), username)
		ch <- prometheus.MustNewConstMetric(m.MinerCheckSectorsChecked, prometheus.CounterValue, float64(sectors.Checked), fmt.Sprintf("%v", deadline), username)
	}
	m.mutex.Lock()
	...
	if 0 < len(minerId) {
		ch <- prometheus.MustNewConstMetric(m.MinerId, prometheus.CounterValue, float64(1), minerId, username)
		...
		if err == nil {
			for dlIdx, deadline := range deadlines.Deadlines {
				...
				ch <- prometheus.MustNewConstMetric(m.ProvingDeadlineAllSectors, prometheus.CounterValue, float64(deadline.AllSectors), fmt.Sprintf("%v", dlIdx), username)
				ch <- prometheus.MustNewConstMetric(m.ProvingDeadlineFaultySectors, prometheus.CounterValue, float64(deadline.FaultySectors), fmt.Sprintf("%v", dlIdx), username)
				ch <- prometheus.MustNewConstMetric(m.ProvingDeadlineCurrent, prometheus.CounterValue, float64(current), fmt.Sprintf("%v", dlIdx), username)
				ch <- prometheus.MustNewConstMetric(m.ProvingDeadlinePartitions, prometheus.CounterValue, float64(deadline.Partitions), fmt.Sprintf("%v", dlIdx), username)
				ch <- prometheus.MustNewConstMetric(m.ProvingDeadlineProvenPartitions, prometheus.CounterValue, float64(deadline.ProvenPartitions), fmt.Sprintf("%v", dlIdx), username)
			}
		} else {
	...
	ch <- prometheus.MustNewConstMetric(m.LogFileSize, prometheus.CounterValue, float64(filesize), username)

	chainSyncNotCompletedHosts := m.ml.GetChainSyncNotCompletedHosts()
	//
	for host := range chainSyncNotCompletedHosts {
		ch <- prometheus.MustNewConstMetric(m.ChainSyncNotCompleted, prometheus.CounterValue, float64(1), host, username)
	}
	chainNotSuitable := m.ml.GetChainNotSuitable()
	ch <- prometheus.MustNewConstMetric(m.ChainNotSuitable, prometheus.CounterValue, float64(chainNotSuitable), username)

	chainHeadListenSuccessHosts := m.ml.GetChainHeadListenSuccessHosts()
	for host, epoch := range chainHeadListenSuccessHosts {
		ch <- prometheus.MustNewConstMetric(m.ChainHeadListen, prometheus.CounterValue, float64(epoch), host, username)
	}

	for k, v := range m.storageStat {
		if v != nil {
			ch <- prometheus.MustNewConstMetric(m.StorageMountError, prometheus.CounterValue, 1, k, username)
		} else {
			ch <- prometheus.MustNewConstMetric(m.StorageMountError, prometheus.CounterValue, 0, k, username)
		}
		filePerm, _ := systemapi.FilePerm2Int(k)
		ch <- prometheus.MustNewConstMetric(m.StorageMountpointPermission, prometheus.CounterValue, float64(filePerm), k, username)
	}

	minerFileOpenNumber, _ := systemapi.GetProcessOpenFileNumber("lotus-miner")
	ch <- prometheus.MustNewConstMetric(m.MinerOpenFileNumber, prometheus.CounterValue, float64(minerFileOpenNumber), username)

	tcpConnectNumber, _ := systemapi.GetProcessTcpConnectNumber("lotus-miner")
	ch <- prometheus.MustNewConstMetric(m.MinerProcessTcpConnectNumber, prometheus.CounterValue, float64(tcpConnectNumber), username)

	ch <- prometheus.MustNewConstMetric(m.MinerAdjustBaseFee, prometheus.CounterValue, minerAdjustBaseFee, username)
	ch <- prometheus.MustNewConstMetric(m.MinerAdjustGasFeecap, prometheus.CounterValue, minerAdjustGasFeecap, username)

	if mineOne.MiningEligible {
		ch <- prometheus.MustNewConstMetric(m.MiningEligible, prometheus.CounterValue, float64(1), username)
	} else {
		ch <- prometheus.MustNewConstMetric(m.MiningEligible, prometheus.CounterValue, float64(0), username)
	}
	if mineOne.MiningLateBase {
		ch <- prometheus.MustNewConstMetric(m.MiningLateBase, prometheus.CounterValue, float64(1), username)
	} else {
		ch <- prometheus.MustNewConstMetric(m.MiningLateBase, prometheus.CounterValue, float64(0), username)
	}
	if mineOne.MiningLateWinner {
		ch <- prometheus.MustNewConstMetric(m.MiningLateWinner, prometheus.CounterValue, float64(1), username)
	} else {
		ch <- prometheus.MustNewConstMetric(m.MiningLateWinner, prometheus.CounterValue, float64(0), username)
	}
	switch mineOne.MiningLateBaseDeltaSecond.(type) {
	case float64:
		ch <- prometheus.MustNewConstMetric(m.MiningLateBaseDeltaSecond, prometheus.CounterValue, mineOne.MiningLateBaseDeltaSecond.(float64), username)
	case int64:
		ch <- prometheus.MustNewConstMetric(m.MiningLateBaseDeltaSecond, prometheus.CounterValue, float64(mineOne.MiningLateBaseDeltaSecond.(int64)), username)
	case string:
		miningLateBaseDeltaSecond, _ := strconv.ParseFloat(mineOne.MiningLateBaseDeltaSecond.(string), 64)
		ch <- prometheus.MustNewConstMetric(m.MiningLateBaseDeltaSecond, prometheus.CounterValue, miningLateBaseDeltaSecond, username)
	}
	...
	ch <- prometheus.MustNewConstMetric(m.MiningNetworkPower, prometheus.CounterValue, miningNetworkPower, username)
	ch <- prometheus.MustNewConstMetric(m.MiningMinerPower, prometheus.CounterValue, miningMinerPower, username)

	for _, path := range m.lotusStoragePath {
		pathStatus := getMinerRepoDirUsage(path)
		ch <- prometheus.MustNewConstMetric(m.MinerRepoDirUsage, prometheus.CounterValue, pathStatus.Used, fmt.Sprintf("%v", path), fmt.Sprintf("%v", pathStatus.All), username)
	}
}
